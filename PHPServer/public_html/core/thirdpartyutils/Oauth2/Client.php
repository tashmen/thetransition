...
class Client
{
    ...
    */
    ...
    public function getAccessToken($token_endpoint, $grant_type, array $parameters)
    {
        ...
        $grantTypeClass =  /*'OAuth2' . '\\GrantType\\' .*/ $grantTypeClassName;
        if (!class_exists($grantTypeClass)) {
            throw new InvalidArgumentException('Unknown grant type \'' . $grant_type . '\'', oauthInvalidArgumentException::INVALID_GRANT_TYPE);
        }
        ...
        $parameters['grant_type'] = constant($grantTypeClass . '::GRANT_TYPE');
        $http_headers = array();
    ...
    private function convertToCamelCase($grant_type)
    {
        ...
        array_walk($parts, 'walkme');
        return implode('', $parts);
    }
	
}
function walkme(&$item) { $item = ucfirst($item);}
